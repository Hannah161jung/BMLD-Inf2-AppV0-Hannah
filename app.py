...
# --- CODE: import and initialize data manager and login manager ---
...
data_manager = DataManager(       # initialize data manager
    ...
    fs_root_folder="App_BMLD_pH"  # folder on switch drive where the data is stored
    ) 
...
# --- CODE: load user data from data manager if not already present in session state --
...
pg_pH_Rechner = st.Page("views/pH_Rechner.py", title="pH-Rechner", icon=":material/science:")

pg = st.navigation([pg_home, pg_pH_Rechner])
pg.run()
